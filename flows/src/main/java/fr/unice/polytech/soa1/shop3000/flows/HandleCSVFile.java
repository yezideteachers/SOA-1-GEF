...
public class HandleCSVFile extends RouteBuilder {
	...
	public void configure() throws Exception {
		...
		from(Endpoints.CSV_INPUT_DIRECTORY)
				...
				.choice()
					.when(header("idMarket").isEqualTo(0))
						...
						.to(Endpoints.HANDLE_ORDER_0)   // Async transfer with JMS ( activemq:handleAOrder0 )
					.when(header("idMarket").isEqualTo(10))
						.log("  Transferring to the route that handle a given market")
						.to(Endpoints.HANDLE_ORDER_1)   // Async transfer with JMS ( activemq:handleAOrder1 )
					.otherwise()
